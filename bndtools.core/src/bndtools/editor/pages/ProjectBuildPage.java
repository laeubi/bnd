...
public class ProjectBuildPage extends FormPage implements IPriority, IResourceChangeListener {
	...
	public static final IFormPageFactory	FACTORY				= new IFormPageFactory() {
																	@Override
																	public IFormPage createPage(
																		ExtendedFormEditor editor, BndEditModel model,
																		String id) throws IllegalArgumentException {
																		return new ProjectBuildPage(editor, model, id,
																			"Build");
																	}
	...
	protected void createFormContent(IManagedForm managedForm) {
		...
		SubBundlesPart subBundlesPart = new SubBundlesPart(leftPanel, tk,
		...
		BuildPathPart buildPathPart = new BuildPathPart(leftPanel, tk,
			ExpandableComposite.TITLE_BAR | ExpandableComposite.EXPANDED | Section.DESCRIPTION);
		...
		BuildOperationsPart buildOpsPart = new BuildOperationsPart(rightPanel, tk,
			ExpandableComposite.TITLE_BAR | ExpandableComposite.EXPANDED | Section.DESCRIPTION);
	...
	private void generateFixes(String message, final IMarker marker) {
		...
		if (proposals != null) {
			for (ICompletionProposal proposal : proposals) {
				fixes.add(
					new ApplyCompletionProposalAction(proposal, editor.getSourcePage(), editor, BndEditor.SOURCE_PAGE));
			}
		}

		// If no source completion fixes were found, add the marker resolution
		// actions.
		if (fixes.isEmpty()) {
	...
	void reportProblemsInHeader() {
		...
		for (Entry<String, Integer> entry : messageSeverityMap.entrySet()) {
			...
			IAction[] fixes = messageFixesMap.get(entry.getKey()); // may be
																	// null
